...
class HomeController extends Controller
{
    ...
    public function load_new_product_section(Request $request)
    {
        $limit = 36;
        ...
        $offset = ($page - 1) * $limit;
        if ($request->has('page')) {
            $new_products = filter_products(Product::query())
                ->inRandomOrder()
                ->skip($offset)
                ->take($limit)
                ->get();
            ...
            $new_products = Cache::remember('new_products_36', 3600, function () use ($limit) {
        ...
        return view(
            'frontend.' . get_setting('homepage_select') . '.partials.new_products_section',
    ...
    public function trackOrder(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $order = Order::where('code', $search)
                ->orWhere('tracking_code', $search)
                ->first();
    
            if ($order) {
                $valueForApi = $order->tracking_code === $search
                    ? $order->tracking_code
                    : $order->code;

                $type = $order->tracking_code === $search ? 'tracking' : 'invoice';
    
                $response = $this->steadfastService->getStatus($type, $valueForApi);
    
                if ($response && ($response['status'] ?? null) == 200) {
                    $steadfastStatus = $response['delivery_status'] ?? 'Pending';
                }
            }
        }
    
        return view('frontend.track_order', compact('order', 'steadfastStatus'));
    }
