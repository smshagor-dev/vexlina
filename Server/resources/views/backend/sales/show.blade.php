                ...
                @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
                    ...
                    <div class="col-md-2 ml-auto">
                        ...
                        @if ($order->delivery_status == 'pending' || $order->delivery_status == 'confirmed')
                            ...
                            @endif
                        @else
                            <input type="text" class="form-control" value="{{ translate(ucfirst($shipping_method)) }}" disabled>    
                        @endif
                    </div>
                    @if ($order->shipping_method == 'shiprocket')
                    <div class="col-md-2 ml-auto">
                    ...
                    @if(!$order->steadfast_synced)
                        <div class="col-md-2 ml-auto d-flex align-items-end">
                            <form action="{{ route('orders.send_to_steadfast', $order->id) }}" method="POST" class="w-100">
                                @csrf
                                <button type="submit"
                                    class="form-control text-white d-flex align-items-center justify-content-center"
                                    style="
                                        ...
                                        cursor: pointer;
                                    ">
                                    <i class="las la-truck mr-1"></i> {{-- Using Line Awesome icon --}}
                                    {{ translate('Send to Stead Fast') }}
                                </button>
                            </form>
                        </div>
                    @else
                    ...
                    {{-- Show Consignment ID --}}
                        <div class="col-md-2 ml-auto">
                            <label for="steadfast_consignment_id">{{ translate('Steadfast Consignment ID') }}</label>
                            <input type="text" class="form-control" id="steadfast_consignment_id"
                                value="{{ $order->steadfast_consignment_id ?? 'Waiting' }}" disabled>
                        </div>
                
                        {{-- Show Live Status --}}
                        <div class="col-md-2 ml-auto">
                            <label for="live_steadfast_status">{{ translate('Live Steadfast Status') }}</label>
                            <input type="text" class="form-control" id="live_steadfast_status"
                                value="{{ $steadfastStatus ? ucfirst(str_replace('_', ' ', $steadfastStatus)) : 'Not available' }}" disabled>
                        </div>
                    @endif
