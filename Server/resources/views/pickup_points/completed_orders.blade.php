...
<div class="card shadow-none rounded-0 border">
    <div class="card-header border-bottom-0">
        <h5 class="mb-0 fs-20 fw-700 text-dark">{{ translate('Completed Orders') }}</h5>
    </div>
    <div class="card-body">
            ...
            <tbody class="fs-14">
                @foreach ($completed_orders as $order)
                    <tr>
                        ...
                        <td>{{ $order->delivered_date ? date('d-m-Y h:i A', strtotime($order->delivered_date)) : date('d-m-Y h:i A', strtotime($order->updated_at)) }}</td>
        ...
        <div class="aiz-pagination mt-2">{{ $completed_orders->appends(request()->input())->links() }}</div>
    </div>
</div>
@endsection
