...
<div class="card shadow-none rounded-0 border">
    <div class="card-header border-bottom-0">
        <h5 class="mb-0 fs-20 fw-700 text-dark">{{ translate('On The Way Orders') }}</h5>
    </div>
    <div class="card-body">
        <table class="table aiz-table mb-0">
            <thead class="text-gray fs-12">
                <tr>
                    ...
                    <th>{{ translate('Accept') }}</th>
                    <th class="text-right pr-0">{{ translate('Options') }}</th>
                </tr>
            </thead>
            <tbody class="fs-14">
                @foreach ($on_the_way_orders as $order)
                    <tr>
                        ...
                        <td>
                            <button type="button" onclick="updatePickupStatus({{ $order->id }}, 'reached')" class="btn btn-success btn-sm">
                                {{ translate('Accept') }}
                            </button>
                        </td>
        ...
        <div class="aiz-pagination mt-2">{{ $on_the_way_orders->appends(request()->input())->links() }}</div>
    </div>
</div>
@endsection
