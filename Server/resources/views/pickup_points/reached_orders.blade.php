                ...
                @foreach ($reached_orders as $order)
                    <tr>
                        ...
                        @php
                            ...
                            $deadline = $reachedAt ? \Carbon\Carbon::parse($reachedAt)->startOfDay()->addDays($holdDays) : null;
                            $daysLeft = $deadline ? max(0, \Carbon\Carbon::today()->diffInDays($deadline, false)) : null;
                            $returnDue = $deadline ? \Carbon\Carbon::today()->greaterThanOrEqualTo($deadline->copy()->startOfDay()) : false;
                        @endphp
                        <td>
                            @if ($returnDue)
                                <span class="badge badge-inline badge-danger">{{ translate('Return Due') }}</span>
                            @else
                                <span class="badge badge-inline badge-warning">{{ $daysLeft }} {{ translate('days left') }}</span>
                            @endif
                        </td>
                        <td>
                            <button type="button" onclick="updatePickupStatus({{ $order->id }}, 'delivered')" class="btn btn-success btn-sm" @if($returnDue) disabled @endif>
                                {{ translate('Complete') }}
                            </button>
                        </td>
                        <td>
