...
class _SingleOrderMapState extends State<SingleOrderMap> {
  ...
  initLocation() {
    final hasCustomerLocation = widget.order.hasCustomerLocation == true;
    final hasStoreLocation = widget.order.hasStoreLocation == true;

    if (hasCustomerLocation) {
      _kMapCenter = LatLng(widget.order.lat!, widget.order.lang!);
    } else if (hasStoreLocation) {
      _kMapCenter = LatLng(widget.order.delivery_pickup_latitude!,
    ...
    if (hasStoreLocation) {
      ...
      _storeMapCenter = _kMapCenter;
    }

    location_initialized = hasCustomerLocation || hasStoreLocation;
    setState(() {});
  }
  ...
  Set<Marker> _createMarker() {
    ...
    if (widget.order.hasCustomerLocation == true) {
      markers.add(
        Marker(
    ...
    if (widget.order.hasStoreLocation == true) {
      markers.add(
        Marker(
  ...
  onPressCenterMap() async {
    if (!_mapControllerReady || !location_initialized) {
      return;
    }
