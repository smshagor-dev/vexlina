...
import 'package:active_flutter_delivery_app/screens/multi_order_map.dart';
...
class _AssignedDeliveryState extends State<AssignedDelivery> {
  ...
  List<Order> getAvailableLocationOrders(List<Order> orders) {
    ...
    orders.forEach((element) {
      if (element.hasAnyDirectionLocation) {
        latLng.add(element);
      }
    });
  ...
  Future<void> _openDirection(dynamic order) async {
    final lat = order.preferredDirectionLatitude;
    final lng = order.preferredDirectionLongitude;
    Uri? googleMapsUri;
    ...
    if (lat != null && lng != null) {
      ...
      final details = await OrderRepository().getOrderDetails(id: order.id);
      final detailedOrder = (details.detailed_orders != null &&
              details.detailed_orders!.isNotEmpty)
          ? details.detailed_orders!.first
          : null;
      final address = detailedOrder?.shipping_address;
    ...
    if (googleMapsUri == null) {
      ToastComponent.showDialog(LangText(context).local!.location_not_available,
          context,
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          // show all delivery point location button
          Container(
            child: deliveryPoints.length > 0
                ? InkWell(
                    ...
                    onTap: () {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context) =>
                                  MultiOrderMap(orders: deliveryPoints)));
                    },
                    child: Row(
                      children: [
                        Text(LangText(context).local!.get_locations,
                            style: TextStyle(
                                fontSize: 14,
                                color: MyTheme.blue,
                                fontWeight: FontWeight.w600)),
